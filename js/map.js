...
/**
 * 处理视域生成模式下的点击
 * 参照图层管理功能：直接遍历 feature.getProperties() 的所有字段
 * 用模糊匹配自动识别视域参数，不再依赖硬编码字段名
 */
async function handleFootprintGenClick(feature, coordinate) {
    console.log('%c[视域生成 v4] 点击处理开始', 'background:#1890ff;color:white;padding:3px 8px;border-radius:3px;');

    // ====== 参照图层管理：获取所有 properties ======
    const props = feature.getProperties();
    const allKeys = Object.keys(props).filter(k => k !== 'geometry').sort();

    console.log('[视域生成 v4] 所有字段名:', allKeys);
    console.log('[视域生成 v4] properties 完整内容:', JSON.parse(JSON.stringify(props)));

    // ====== 遍历所有字段，用模糊匹配自动识别视域参数 ======
    const params = scanParamsFromProperties(props);

    // 坐标回退：properties 没有 → geometry → 点击坐标
    if (params.lat == null || params.lon == null) {
        if (typeof feature.getGeometry === 'function') {
            const geom = feature.getGeometry();
            if (geom && geom.getType() === 'Point') {
                try {
                    const c = ol.proj.toLonLat(geom.getCoordinates());
                    params.lon = c[0]; params.lat = c[1];
                    params.latLonSource = 'geometry(Point)';
                    params.fieldMap.lat = 'geometry';
                    params.fieldMap.lon = 'geometry';
                } catch (e) {}
            }
        }
        if ((params.lat == null || params.lon == null) && coordinate) {
            const c = ol.proj.toLonLat(coordinate);
            params.lon = c[0]; params.lat = c[1];
            params.latLonSource = '点击坐标(回退)';
        }
    }

    const filename = props.filename || '';
    console.log('[视域生成 v4] 识别到的参数:', params);

    // ====== 如果参数不完整，尝试从后端补全 ======
    const missingFromFrontend = checkMissingParams(params);
    if (missingFromFrontend.length > 0 && filename) {
        console.log('[视域生成 v4] 前端缺少:', missingFromFrontend, '→ 从后端补全');
        showPopupLoading(filename, coordinate);
        ...
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/api/photos/${encodeURIComponent(filename)}`, { headers });
            if (res.ok) {
                const photoData = await res.json();
                console.log('[视域生成 v4] 后端返回:', photoData);

                // 用同样的模糊匹配从后端数据提取
                const backendParams = scanParamsFromProperties(photoData);
                ...
                console.log('[视域生成 v4] 后端补全后:', params);
            } else {
                console.warn('[视域生成 v4] 后端请求失败:', res.status);
            }
        } catch (e) {
            console.warn('[视域生成 v4] 后端获取异常:', e.message);
        }
    }

    // ====== 构建弹窗 ======
    const missingParams = checkMissingParams(params);
    const paramsComplete = missingParams.length === 0;

    window.__pendingFootprint = { params, filename, coordinate };

    window.__fpGenerate = function() {
        const { params: p, filename: fn } = window.__pendingFootprint;
        const result = generateFootprint(p, fn);
        const panel = document.getElementById('fpResultPanel');
        if (panel) {
            panel.innerHTML = result.html;
            panel.style.cssText = 'margin-top:8px;' + result.style;
        }
        const genBtn = document.getElementById('fpGenerateBtn');
        if (genBtn) {
            genBtn.disabled = true;
            genBtn.style.opacity = '0.5';
            genBtn.style.cursor = 'not-allowed';
            genBtn.innerText = '✅ 已生成';
        }
    };
    ...
    window.__fpClearCurrent = function() {
        if (footprintGenLayer) {
            const count = footprintGenLayer.getSource().getFeatures().length;
            footprintGenLayer.getSource().clear();
            showMessage(count > 0 ? `已清除 ${count} 个视域` : '当前无可清除视域', count > 0 ? 'success' : 'info');
            const panel = document.getElementById('fpResultPanel');
            if (panel) {
                panel.innerHTML = '<div style="font-weight:bold;">🧹 已清除视域</div>';
                panel.style.cssText = 'margin-top:8px; background:#e6f7ff; border:1px solid #91d5ff; border-radius:4px; padding:10px; font-size:13px; color:#0050b3;';
            }
            const genBtn = document.getElementById('fpGenerateBtn');
            if (genBtn && paramsComplete) {
                genBtn.disabled = false;
                genBtn.style.opacity = '1';
                genBtn.style.cursor = 'pointer';
                genBtn.innerText = '🎯 生成视域';
            }
        }
    };

    // ====== 构建全字段表格（参照图层管理，直接遍历所有 properties） ======
    let propRowsHtml = '';
    for (const k of allKeys) {
        let val = props[k];
        let displayVal, valType;
        if (val === null) {
            displayVal = '<span style="color:#ccc;">null</span>';
            valType = 'null';
        } else if (val === undefined) {
            displayVal = '<span style="color:#ccc;">undefined</span>';
            valType = 'undefined';
        } else if (typeof val === 'object') {
            try {
                const str = JSON.stringify(val);
                displayVal = '<code style="font-size:11px;color:#888;">' + str.substring(0, 120) + (str.length > 120 ? '...' : '') + '</code>';
            } catch (e) {
                displayVal = String(val).substring(0, 120);
            }
            valType = typeof val;
        } else {
        ...
        // 检查这个字段是否被模糊匹配识别为视域参数
        const matchedParam = Object.entries(params.fieldMap || {}).find(([_, key]) => key === k);
        const isMatched = matchedParam && matchedParam[1] === k;
        const hasValue = val !== null && val !== undefined && val !== '';
        const rowBg = isMatched ? 'background:#f6ffed;' : (hasValue ? '' : 'background:#fafafa;');
        const flag = isMatched ? ' <span style="color:#52c41a;font-weight:bold;">✓' + matchedParam[0] + '</span>' : '';
        const typeBadge = `<span style="font-size:9px;color:#999;background:#f0f0f0;padding:1px 4px;border-radius:2px;">${valType}</span>`;
        propRowsHtml += `<tr style="${rowBg}">
            <td style="padding:4px 8px;border:1px solid #eee;font-weight:bold;white-space:nowrap;">${k}${flag}</td>
            <td style="padding:4px 8px;border:1px solid #eee;">${displayVal}</td>
            <td style="padding:4px 8px;border:1px solid #eee;font-size:10px;">${typeBadge}</td>
        </tr>`;
    }
    ...
    const resultPanelHtml = paramsComplete
        ? '<div style="font-size:12px;color:#666;">参数完整，点击下方"生成视域"按钮绘制视域范围</div>'
        : `<div style="background:#fff3cd;border:1px solid #ffc107;border-radius:4px;padding:10px;font-size:12px;color:#856404;">
              <div style="font-weight:bold;margin-bottom:4px;">⚠️ 缺少必要参数:</div>
              <ul style="margin:0;padding-left:20px;">${missingParams.map(p => `<li>${p}</li>`).join('')}</ul>
           </div>`;

    const content = `
        <div style="position:relative;padding-top:5px;min-width:360px;max-width:560px;">
            ...
            <h4 style="margin:0 0 8px 0;font-size:14px;">🎯 视域生成 v4 &nbsp;|&nbsp; ${filename || '未命名'}</h4>
            ${summaryHtml}
            <div id="fpResultPanel" style="margin-top:8px;">${resultPanelHtml}</div>
            <div style="margin-top:10px;display:flex;gap:6px;flex-wrap:wrap;padding-top:8px;border-top:1px dashed #ddd;">
                ${paramsComplete
                    ? '<button id="fpGenerateBtn" onclick="window.__fpGenerate()" style="flex:1;min-width:140px;padding:8px 16px;border:1px solid #52c41a;background:#f6ffed;color:#389e0c;border-radius:4px;cursor:pointer;font-size:13px;font-weight:bold;">🎯 生成视域</button>'
                    : '<button disabled style="flex:1;min-width:140px;padding:8px 16px;border:1px solid #d9d9d9;background:#f5f5f5;color:#bfbfbf;border-radius:4px;font-size:13px;cursor:not-allowed;">参数不完整</button>'
                }
            ...
            <!-- 全字段表格（参照图层管理，遍历 feature.getProperties()） -->
            <div style="margin-top:10px;border-top:1px dashed #ddd;padding-top:8px;">
                <div style="font-size:12px;font-weight:bold;margin-bottom:6px;color:#333;">
                    📋 全部字段 (${allKeys.length}个)
                    <span style="font-size:11px;font-weight:normal;color:#888;">（绿色=已匹配视域参数，灰色=值为空）</span>
                </div>
                <div style="overflow:auto;max-height:300px;border:1px solid #e8e8e8;border-radius:4px;">
                    <table style="width:100%;border-collapse:collapse;font-size:12px;">
                        <thead><tr style="background:#fafafa;position:sticky;top:0;">
                            ...
                            <th style="padding:4px 8px;border:1px solid #eee;text-align:left;width:50px;">类型</th>
                        </tr></thead>
...
/**
 * 参照图层管理方式：遍历 properties 的所有字段
 * 用模糊匹配（key.toLowerCase().includes()）自动识别视域参数
 * 不再依赖硬编码的候选键列表
 */
function scanParamsFromProperties(props) {
    const result = {
        lat: null, lon: null, yaw: null, pitch: null, roll: null,
        relHeight: null, hFov: null, vFov: null, deviceType: '',
    ...
    for (const key of Object.keys(props)) {
        if (key === 'geometry' || key === 'layer') continue;
        const val = props[key];
        ...
        // lat（纬度）：key 包含 'lat' 但不包含 'lon'/'lng'
        if (result.lat == null && lk.includes('lat') && !lk.includes('lon') && !lk.includes('lng')) {
            const n = toNumber(val);
            if (n != null) { result.lat = n; result.fieldMap.lat = key; }
        }
        // lon（经度）：key 包含 'lon' 或 'lng'
        if (result.lon == null && (lk.includes('lon') || lk.includes('lng'))) {
            const n = toNumber(val);
            if (n != null) { result.lon = n; result.fieldMap.lon = key; }
        }
        // yaw（偏航角）：key 包含 'yaw'/'heading'/'azimuth'/'bearing'/'方位'/'偏航'/'航向'
        if (result.yaw == null && (lk.includes('yaw') || lk.includes('heading') || lk.includes('azimuth') || lk.includes('bearing') || lk.includes('方位') || lk.includes('偏航') || lk.includes('航向'))) {
            let n = toNumber(val);
            if (n != null) {
                if (n < 0) n += 360; // 负值转 0~360
                result.yaw = n; result.fieldMap.yaw = key;
            }
        }
        // pitch（俯仰角）：key 包含 'pitch'/'tilt'/'俯仰'/'倾斜'/'俯视'
        if (result.pitch == null && (lk.includes('pitch') || lk.includes('tilt') || lk.includes('俯仰') || lk.includes('倾斜') || lk.includes('俯视'))) {
            const n = toNumber(val);
            if (n != null) { result.pitch = n; result.fieldMap.pitch = key; }
        }
        // roll（翻滚角）：key 包含 'roll'/'翻滚'/'横滚'
        if (result.roll == null && (lk.includes('roll') || lk.includes('翻滚') || lk.includes('横滚'))) {
            const n = toNumber(val);
            if (n != null) { result.roll = n; result.fieldMap.roll = key; }
        }
        // relHeight（相对高度）：key 包含 'height'/'altitude'/'alt'/'高度'/'海拔'/'elev'
        //   排除 hfov/vfov 中的 'h'/'v' 干扰
        if (result.relHeight == null && !lk.includes('fov') &&
            (lk.includes('height') || lk.includes('altitude') || lk.includes('alt') ||
             lk.includes('高度') || lk.includes('海拔') || lk.includes('elev'))) {
            const n = toNumber(val);
            if (n != null && n > 0) { result.relHeight = n; result.fieldMap.relHeight = key; }
        }
        // hFov（水平视场角）：key 包含 'h_fov'/'hfov'/'horizontal_fov'/'水平'/'视场角h'
        if (result.hFov == null && (lk.includes('h_fov') || lk.includes('hfov') ||
             lk.includes('horizontal_fov') || lk.includes('水平视场') || lk.includes('视场角h') || lk.includes('horiz_fov'))) {
            const n = toNumber(val);
            if (n != null) { result.hFov = n; result.fieldMap.hFov = key; }
        }
        // vFov（垂直视场角）：key 包含 'v_fov'/'vfov'/'vertical_fov'/'垂直'/'视场角v'
        if (result.vFov == null && (lk.includes('v_fov') || lk.includes('vfov') ||
             lk.includes('vertical_fov') || lk.includes('垂直视场') || lk.includes('视场角v') || lk.includes('vert_fov'))) {
            const n = toNumber(val);
            if (n != null) { result.vFov = n; result.fieldMap.vFov = key; }
        }
        // deviceType（设备类型）：key 包含 'device'/'设备'/'型号'/'model'
        if (!result.deviceType && (lk.includes('device') || lk.includes('设备') || lk.includes('型号') || lk === 'model')) {
            result.deviceType = String(val);
            result.fieldMap.device = key;
        }
    }
 ...
 * 检查缺失的视域参数
 */
function checkMissingParams(p) {
    const missing = [];
    if (p.lat == null || p.lon == null) missing.push('定位坐标(lat/lon)');
    if (p.yaw == null) missing.push('偏航角(yaw)');
    if (p.pitch == null) missing.push('俯仰角(pitch)');
    if (p.relHeight == null || p.relHeight <= 0) missing.push('相对高度(>0m)');
    if (p.hFov == null) missing.push('水平视场角(h_fov)');
    if (p.vFov == null) missing.push('垂直视场角(v_fov)');
    return missing;
}
 ...
 * 显示加载中弹窗
 */
function showPopupLoading(filename, coordinate) {
    popup.getElement().innerHTML = `
        <div style="padding:15px;min-width:280px;text-align:center;">
            <div style="font-size:14px;font-weight:bold;margin-bottom:8px;">📡 从后端获取参数...</div>
            <div style="font-size:12px;color:#888;">${filename}</div>
            <div style="margin-top:10px;color:#1890ff;">⏳ 请稍候</div>
        </div>`;
    popup.setPosition(coordinate);
    popup.setPositioning('top-center');
    popup.setOffset([0, 10]);
 ...
 * 实际生成视域并绘制到地图
 */
function generateFootprint(params, filename) {
    try {
        const isPhone = String(params.deviceType).includes('phone') || String(params.deviceType).includes('footprint') || String(params.deviceType).includes('手机');
        const rollVal = params.roll != null ? params.roll : 0;
        const corners = computeFootprintCorners(params.lat, params.lon, params.relHeight, params.yaw, params.pitch, params.hFov, params.vFov, isPhone, rollVal);

        footprintGenLayer.getSource().clear();

        let footprintCoords, hasCorners, message;
        if (corners === null) {
            footprintCoords = makeCircularFootprint(params.lat, params.lon);
            hasCorners = false;
            message = '⚠️ 降级为圆形视域（全朝天或俯仰角过大）';
        } else {
            footprintCoords = [corners.bottom_left, corners.bottom_right, corners.top_right, corners.top_left, corners.bottom_left];
            hasCorners = true;
            message = '✅ 视域生成成功（4角点）';
        }

        const projectedCoords = footprintCoords.map(([lon, lat]) => ol.proj.fromLonLat([lon, lat]));
        const polygon = new ol.geom.Polygon([projectedCoords]);
        const footprintFeature = new ol.Feature({ geometry: polygon, filename: filename, hasCorners: hasCorners });
        footprintGenLayer.getSource().addFeature(footprintFeature);

        console.log(`[视域生成] ${filename}: ${message}`);

        const style = hasCorners
        ...
        if (hasCorners) {
            const c = corners;
            cornersInfo = `<div style="margin-top:6px;font-size:11px;font-family:monospace;opacity:0.8;">
                ...
                TL:[${c.top_left[1].toFixed(5)},${c.top_left[0].toFixed(5)}]</div>`;
        }
        return { html: `<div style="font-weight:bold;">${message}</div>${cornersInfo}`, style };
    } catch (e) {
        console.error('[视域生成] 计算错误:', e);
        return { html: `<div style="font-weight:bold;">❌ 计算错误: ${e.message}</div>`, style: 'background:#f8d7da;border:1px solid #f5c6cb;border-radius:4px;padding:10px;font-size:13px;color:#721c24;' };
    }
}
...
let cloudFadeTimer = null;
const CLOUD_OPACITY = 0.5;
const CLOUD_FADE_MS = 450;   // 图层切换淡入淡出时长（ms）
const CLOUD_TRANSITION = 200;// 单个图层内部瓦片淡入（避免突兀的瓦片闪现）
 ...
 */
/* ==================== 卫星云图 NASA GIBS 图层模式（已通过浏览器真实 Image() 探测验证 ✅）
 *
 * ⚠️ 重大修正（2026-08-16 暴力探测确认）：
 *  NASA GIBS 并不提供 "Himawari_AHI_CorrectedReflectance_TrueColor" 这种产品
 *  EPSG3857 下 Himawari 只有 3 个产品：Band3_Red_Visible_1km、Band13_Clean_Infrared、Air_Mass
 *  能做到"每小时真彩色"的地球静止卫星是 GOES-East（美洲）/GOES-West（太平洋）的 ABI_GeoColor（png）
 *  精确 TileMatrixSet 与 Format 通过 WMTS GetCapabilities XML 解析确认，不是靠猜。
 */
const SATELLITE_MODES = {
    daily: {
        name: '全球观测（VIIRS 每日·无缝✅推荐）',
        description: '🌍 VIIRS SNPP 真彩色（每日合成·完全无缝）/ 每天 1 帧 / 24 天',
    ...
    daily_modis_aqua: {
        ...
        description: '🌍 MODIS Aqua 真彩色 / 每天 1 帧 / 24 天（注意：极地轨道条带色差缝）',
    ...
    daily_modis_8day: {
        ...
        description: '🌍 MODIS Aqua 真彩色 8天合成 / 每 8 天 1 帧 / ≈192 天跨度（多轨道平均消色差）',
    ...
    // ======== 以上：日级真彩色 ======== | ======== 以下：地球静止卫星 每小时级 ========
    goes_east_geocolor: {
        name: '美洲·GOES-East（每小时真彩色·GeoColor ✅）',
        description: '🌎 GOES-16 静止卫星 GeoColor 真彩色（覆盖南美/北美/大西洋）/每小时1帧/24小时',
        ...
        delayHours: 3,
        gutterPx: 1,
        timeFormat: (d) => { d.setUTCMinutes(0, 0, 0); return d.toISOString().replace(/\.\d+Z$/, 'Z'); },
        displayFormat: (d) => (d.getMonth()+1)+'/'+d.getDate()+' '+String(d.getHours()).padStart(2,'0')+':00',
        marks: ['-24h', '-18h', '-12h', '-6h', '最新']
    },
    goes_west_geocolor: {
        name: '太平洋·GOES-West（每小时真彩色·GeoColor ✅）',
        description: '🌎 GOES-17/18 静止卫星 GeoColor（覆盖太平洋·北美西岸·夏威夷）/每小时1帧/24小时',
    ...
    himawari_band3_red: {
        name: '亚太·葵花（每小时红可见·单通道 ✅）',
        description: '🌏 Himawari 葵花 AHI Band3 640nm 红光（仅单通道灰度图·覆盖亚太）/每小时1帧/24小时',
    ...
    himawari_band13_ir: {
        name: '亚太·葵花（每小时云顶红外 ✅）',
        description: '🌏 Himawari 葵花 AHI Band13 10.4µm 清洁红外亮温（冷暖云色·看台风强对流）/每小时1帧/24小时',
...
let currentSatelliteMode = 'daily'; // daily | hourly

/** 根据当前模式生成时间列表 */
function getGibsDateList(mode) {
    const cfg = SATELLITE_MODES[mode || currentSatelliteMode];
    const out = [];
    const now = new Date();
...
function createSatelliteCloudLayer(dateStr, layerName, tileMatrixSet, ext, gutterPx, initialOpacity) {
    ...
    const initOpacity = (initialOpacity != null) ? initialOpacity : CLOUD_OPACITY;
    ...
    return new ol.layer.Tile({
        source: new ol.source.XYZ({
            ...
            tileSize: 256,
            maxZoom: maxZoom,
            gutter: gutter,        // ← 瓦片外围多抓 gutter px，然后丢弃边缘，消除瓦片 1px 缝
            transition: CLOUD_TRANSITION, // ← 图层内部瓦片淡入（不是 0），柔和刷新
...
/** 线性动画从 fromOpacity 到 toOpacity（ms 内完成） */
function animateLayerOpacity(layer, fromOpacity, toOpacity, durationMs, onDone) {
    const start = performance.now();
    function tick(now) {
        const t = Math.min(1, (now - start) / durationMs);
        const eased = 1 - Math.pow(1 - t, 3); // easeOutCubic，收尾自然
        const cur = fromOpacity + (toOpacity - fromOpacity) * eased;
        try { layer.setOpacity(cur); } catch (e) {}
        if (t < 1) {
            cloudFadeTimer = requestAnimationFrame(tick);
        } else {
            cloudFadeTimer = null;
            if (onDone) onDone();
        }
...
function cancelCloudFade() {
    if (cloudFadeTimer) {
        cancelAnimationFrame(cloudFadeTimer);
        cloudFadeTimer = null;
    }
}
...
function toggleSatelliteCloud() {
    if (satelliteCloudVisible) hideSatelliteCloud();
    else showSatelliteCloud();
}
...
function showSatelliteCloud() {
    ...
    // 初次显示：从 0 淡入 CLOUD_OPACITY，避免地图上突然一闪
    satelliteCloudLayer = createSatelliteCloudLayer(
        item.dateStr, item.layer, item.tileMatrixSet, item.ext, item.gutterPx, 0
    );
    map.addLayer(satelliteCloudLayer);
    animateLayerOpacity(satelliteCloudLayer, 0, CLOUD_OPACITY, CLOUD_FADE_MS * 1.2);
...
function hideSatelliteCloud() {
    ...
    if (satelliteCloudLayer) {
        // 先淡出再移除，避免突然消失
        const oldLayer = satelliteCloudLayer;
        satelliteCloudLayer = null; // 先置空，防止动画期间被当作当前图层
        animateLayerOpacity(oldLayer, CLOUD_OPACITY, 0, CLOUD_FADE_MS, () => {
            try { map.removeLayer(oldLayer); } catch (e) {}
        });
    }
    satelliteCloudVisible = false;
    // 面板也做 200ms 渐隐后隐藏，避免突兀
    const panel = document.getElementById('satellitePanel');
    if (panel) {
...
/** 切换模式（daily ↔ hourly） */
function switchSatelliteMode(mode) {
    if (!SATELLITE_MODES[mode]) return;
    ...
    if (satelliteCloudVisible) {
        ...
        updateCloudTimeLabel(23);
        updateCloudTimeMarks();
    }
}

function updateCloudModeButtons() {
    const cfg = SATELLITE_MODES[currentSatelliteMode];
    ...
    const idleStyle = 'flex:1;padding:4px 8px;font-size:12px;border:1px solid #444;background:rgba(255,255,255,0.06);color:#bbb;border-radius:4px;cursor:pointer;';
    if (dailyBtn) dailyBtn.style.cssText = currentSatelliteMode === 'daily' ? activeStyle : idleStyle;
    if (hourlyBtn) hourlyBtn.style.cssText = currentSatelliteMode === 'hourly' ? activeStyle : idleStyle;
}
...
function updateCloudTimeMarks() {
    ...
    if (!marks) return;
    marks.innerHTML = cfg.marks.map(t => `<span>${t}</span>`).join('');
}
...
/**
 * 平滑切换卫星时间帧：
 *   1. 旧图层 450ms 淡出到 0 → 移除
 *   2. 新图层 opacity=0 立即加入 → 450ms 淡入到 CLOUD_OPACITY
 * 旧图层不会立即消失，所以中间没有空白/闪烁。
 */
function updateCloudLayer(sliderIdx) {
    ...
    // --- 建新图层（opacity=0，等待瓦片+淡入）---
    const newLayer = createSatelliteCloudLayer(
        item.dateStr, item.layer, item.tileMatrixSet, item.ext, item.gutterPx, 0
    );
    map.addLayer(newLayer);
    satelliteCloudLayer = newLayer;

    // --- 新图层淡入 ---
    animateLayerOpacity(newLayer, 0, CLOUD_OPACITY, CLOUD_FADE_MS);
...
function initSatelliteCloud() {
    ...
    // ===== 高级模式开关：精简版（默认只有 VIIRS）/ 完整版（含多个备选）=====
    let cloudAdvancedShown = false;
    const CLOUD_MODE_LIST = [
        { key: 'daily',         default: true,  label: '⭐ VIIRS SNPP（全球每日 · 完全无缝 · 推荐）', advanced: false, group: '日级' },
        // 日级备选
        { key: 'daily_modis_8day',  label: '🌍 MODIS Aqua 8天合成（全球·无缝·192天跨度）',            advanced: true,  group: '日级' },
        { key: 'daily_modis_aqua',  label: '🌍 MODIS Aqua 每日（下午过境·有轨道色差缝）',              advanced: true,  group: '日级' },
        // 每小时静止卫星（逐个真实瓦片验证通过 ✅）
        { key: 'goes_east_geocolor', label: '🌎 GOES-East GeoColor（美洲·每小时真彩色·-3h延迟）',       advanced: true,  group: '每小时' },
        { key: 'goes_west_geocolor', label: '🌎 GOES-West GeoColor（太平洋·每小时真彩色·-3h延迟）',      advanced: true,  group: '每小时' },
        { key: 'himawari_band3_red', label: '🌏 葵花8号 Band3 红光（亚太·每小时·单通道灰度·-3h）',       advanced: true,  group: '每小时' },
        { key: 'himawari_band13_ir', label: '🌏 葵花8号 Band13 云顶红外（亚太·每小时·看台风·-3h）',      advanced: true,  group: '每小时' }
    ];
    function rebuildModeSelect() {
        const modeSelect = document.getElementById('cloudModeSelect');
        if (!modeSelect) return;
        const saved = modeSelect.value || currentSatelliteMode;
        modeSelect.innerHTML = '';

        // --- 默认星源组（精简/完整版都有）---
        const gDefault = document.createElement('optgroup');
        gDefault.label = '⭐ 主星源（无缝·日更新·推荐）';
        CLOUD_MODE_LIST.filter(m => !m.advanced).forEach(m => {
            const op = document.createElement('option');
            op.value = m.key; op.textContent = m.label;
            gDefault.appendChild(op);
        });
        modeSelect.appendChild(gDefault);

        // --- 备选星源组（只在展开 advanced 时显示，按分组分类）---
        if (cloudAdvancedShown) {
            ['日级','每小时'].forEach(groupName => {
                const items = CLOUD_MODE_LIST.filter(m => m.advanced && m.group === groupName);
                if (!items.length) return;
                const g = document.createElement('optgroup');
                g.label = (groupName === '每小时')
                    ? '🔧 备选 · 每小时级（静止卫星）'
                    : '🔧 备选 · 日级（合成真彩色）';
                items.forEach(m => {
                    const op = document.createElement('option');
                ...
                modeSelect.appendChild(g);
            });
        }

        // 恢复用户上次选的值。若因 advanced 折叠不存在，或指向已删除的旧 key（比如 hourly），就回到 daily
        const stillExists = CLOUD_MODE_LIST.find(m =>
            m.key === saved && (cloudAdvancedShown || !m.advanced)
        );
        if (stillExists) {
            modeSelect.value = saved;
        } else {
            modeSelect.value = 'daily';
            currentSatelliteMode = 'daily';
        }
    }
    rebuildModeSelect();

    const advBtn = document.getElementById('cloudAdvancedToggle');
    const advHint = document.getElementById('cloudAdvancedHint');
    if (advBtn) {
        advBtn.addEventListener('click', () => {
            cloudAdvancedShown = !cloudAdvancedShown;
            rebuildModeSelect();
            if (advBtn) advBtn.textContent = cloudAdvancedShown ? '⬆ 收起模式' : '⚙️ 更多模式';
            if (advHint) advHint.style.display = cloudAdvancedShown ? 'block' : 'none';
        });
    }
    ...
    const hourlyBtn = document.getElementById('cloudModeHourlyBtn');
    if (hourlyBtn) hourlyBtn.addEventListener('click', () => switchSatelliteMode('himawari_band13_ir'));

    // 时间轴
    const slider = document.getElementById('cloudTimeSlider');
    if (slider) {
        slider.addEventListener('input', (e) => updateCloudLayer(parseInt(e.target.value)));
    }
    const prevBtn = document.getElementById('cloudPrevBtn');
    if (prevBtn) prevBtn.addEventListener('click', () => {
        const slider = document.getElementById('cloudTimeSlider');
        const idx = Math.max(0, parseInt(slider.value) - 1);
        slider.value = idx;
    ...
    if (nextBtn) nextBtn.addEventListener('click', () => {
        const slider = document.getElementById('cloudTimeSlider');
        const idx = Math.min(23, parseInt(slider.value) + 1);
        slider.value = idx;
        updateCloudLayer(idx);
    });
